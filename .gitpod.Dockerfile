...
RUN stream-dl --init
RUN wget -q -O /usr/bin/get-hashi https://raw.githubusercontent.com/da-moon/core-utils/master/bin/stream-dl/get-hashi
RUN chmod +x "/usr/bin/get-hashi"
RUN get-hashi
RUN wget -q -O /usr/bin/run-sc https://raw.githubusercontent.com/da-moon/core-utils/master/bin/stream-dl/run-sc
