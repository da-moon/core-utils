...
shellcheck --version
echo
printf "Shellcheck is scanning (%s) files:n" "$#"
printf " %sn" "$@"
echo
set +e
IFS=$'n' results=$(shellcheck --exclude=SC1117 --external-sources --format="$format" "$@" 2>&1)
exit_code=$?
set -e
case "$exit_code" in
0)
...
;;
1)
printf "All files successfully scanned with some issues (%s):n" ${#results[@]}
printf " %sn" "${results[@]}"
exit $exit_code
;;

2)
printf "the following files could not be processed (%s):n" ${#results[@]}
printf " %sn" "${results[@]}"
exit $exit_code
;;
...
local line
WORKING_DIR="$(
readlink -f "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/"
)"
if [[ -z "$filename" ]]; then
